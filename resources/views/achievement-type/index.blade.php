...
@section('content')
    <div class="w-full">
        <div class="flex flex-row justify-end">
            <a href="{{ route('achievement-types.create')  }}" class="btn btn-primary my-2">Tambah</a>
        </div>
                        ...
                        <td class="flex flex-row gap-2 h-full">
                            ...
                            <div class="modal" id="modal-delete-{{ $loop->iteration }}">
                                <div class="modal-box w-5/12 max-w-5xl">
                                    <form action="{{ route('achievement-types.destroy', $type->id) }}"
                                          method="post">
                                        @csrf
                                        @method('delete')

                                        <h1 class="font-bold text-xl text-center">Hapus Jenis Pencapaian</h1>
                                        <h2 class="text-center">
                                            Apakah anda yakin menghapus data jenis pencapaian ini
                                        </h2>
